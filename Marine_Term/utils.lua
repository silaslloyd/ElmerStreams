...
function TDepSlidingExp(C,Th,gamma, noise)
  Th = Th - 273.15
...
function Accumulation(y, t,x, a, W , L)
   acc = a--*(1+2*math.exp(-(x/L)^2)*math.exp(-(y-W/2)^2/(W/6)^2)*(1-math.tanh((t-500)/500))/2)
   return acc
end


function shelf_melt(dist)
   if dist > 0 then  
    m = 100*(math.tanh((dist-50000)/20000)+1)/2
   else 
    m = 0
end
return m
