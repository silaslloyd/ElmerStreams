#!/bin/bash
#SBATCH --output=./slurms/Stream_slurm-%j.out
...
python3 Initialisation/perlin.py
...
if [[ "$MODE" == "periodic" ]]; then
	ElmerGrid 14 2 footprint.msh -partition $((SLURM_NTASKS / 4)) 4 1 -periodic 0 1 0 -autoclean # > /dev/null 2>&1
	ElmerGrid 14 5 footprint.msh -partition $((SLURM_NTASKS / 4)) 4 1 -periodic 0 1 0 -autoclean # > /dev/null 2>&1
...
nohup mpirun -np $nodes ElmerSolver "${SIF_FILE}" > ThermoCoupledScriptOutputs/${NAME}.out 
echo "DONE"
