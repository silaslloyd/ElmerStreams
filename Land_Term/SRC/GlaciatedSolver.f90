...
!******************************************************************************
  ...
  TYPE(Variable_t), POINTER :: PointerToVariable, bedrockVar, FrontVar, ZSvar
  TYPE(Nodes_t), SAVE :: Nodes

  LOGICAL :: AllocationsDone = .FALSE., GotIt, stat,UnFoundFatal=.TRUE.,&
             AllGrounded = .FALSE., useZSvar = .FALSE., Active

  INTEGER :: i, mn, n, t, Nn, istat, DIM, MSum, ZSum, bedrockSource, k
  INTEGER, POINTER :: Permutation(:), bedrockPerm(:), ZSvarPerm(:)
...
!  NULLIFY(bedrockPerm,bedrockVar)
  ...
  IF(.NOT. AllGrounded) THEN
    ...
    ZSvarName = GetString(SolverParams, 'top surface variable', GotIt)
    IF (GotIt) THEN
  ...
  DO t = 1, Solver % NumberOfActiveElements
     ...
     DO i = 1, n
        ...
        IF (useZSvar) THEN
          ZSvar => VariableGet(Model % Mesh % Variables, ZSvarName, UnFoundFatal=UnFoundFatal)
          ZSvarPerm => ZSvar % Perm
          z = ZSvar % values( ZSvarPerm(Element % NodeIndexes(i)) )
        ELSE
          IF (DIM == 2) THEN
            z = Nodes % y( i )
          ELSE IF (DIM == 3) THEN
