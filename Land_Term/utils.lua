...
function TDepSlidingExp(C,Th,gamma, noise,dist)
  Th = Th - 273.15
  Th = IfThenElse(Th > 0.0, 0.0, Th)
  C = C*(1+0*noise/20)
  factor = 1.0 --(mask+1.0)/2 --IfThenElse(mask<=0.0,0.000001,1.0)
  slid = factor*C / math.exp(Th/gamma)
  if (dist < 10000) then slid = C/2 end
 return slid
end
...
function InitialSheet(x,gm,ct,L)
  return 1e-4 + (0.1-1e-4)*(ct+1)/2
end

function getmeltrate(loads,weights,friction, gm, time,y,W)
  --y_spacing  = W/y_nodes
  
  if gm <= 0.0 then
    return 0.0
  else
    melt =  -(loads)/(weights*rhoi*Lw) --weights*
...
function frictionloads(lx,ly,lz,vx,vy,vz, y,W)
  y_spacing = W/y_nodes
  cx = max(-lx*vx,0)
  cy = max(-ly*vy,0)
  cz = max(-lz*vz,0)
...
function Accumulation(y, t,x, a, W , L)
-- acc = 4*a*(L/2-x)/L   ---(1+2*math.exp(-(x/L)^2)*math.exp(-(y-W/2)^2/(W/6)^2)*(1-math.tanh((t-500)/500))/2)
  -- acc = 4*a*((math.tanh((0.75*L-x)/0.1*L)+1)/2)-5*a  
  acc = (x-0.4*L)*2*a/(-0.4*L)
  if x > 0.9*L then
     acc = -5*a
  end
 -- if x < 3/4 *L then 
 ...
 -- else 
 --   b = a*(0.9-9/4)/(0.9-0.8)
 --   acc = -(a-b)/(0.9-0.8)/L*(x-0.8*L)+a
 -- end 
 return acc
end

function surfaceT(height, t, zscale, Tsurf)
    --Tsurf = Tsurf -1
    height = height - 10*zscale
    height = max(height, 0)
    heighrt = min(height, 5000)
    if height > 100 then 
      T = Tsurf
    else
      T = -(273.15 - Tsurf)/100 * height + 273.15 
    end
    if T > 273.15 then 
       T = 273.15
    end
...
function tempbc(gm,ct,dist)
cond = -1
 if gm < 0 then cond = 1
   end
if ct > 0 then cond = 1
end
if dist < 10000 then cond = 1
end
return cond
end
