...
!------------------------------------------------------------------------------    
    ...
    INTEGER :: c,i,j,k,l,m,p,q,t,ngp,norm_comp,no_slip_comp
    ...
    TYPE(Variable_t), POINTER :: citer, iterconverged
...
!------------------------------------------------------------------------------
    ...
    citer => VariableGet( CurrentModel % Mesh % Variables,'coupled iter',ThisOnly=.TRUE.)
    CALL GetLocalSolution( PrevSol, tStep=-1 )
    CALL GetLocalSolution( NodalSol )
            ...
            DO i=1,dim
              ...
              DO q=1,nd
                DO j=1,dim
                  ...
                  FORCE(l) = FORCE(l) + s * Basis(q) * SurfaceTraction(i) * Vect(j)
                  IF (i==1) THEN
                    IF (citer % Values(1) .EQ. 1 ) THEN
                      FORCE(l) = FORCE(l) + s * FSSAtheta * dt * FSSAaccum * Basis(q) * LoadVec(j)
                    ELSE
                      DO m=1,dim
                        FORCE(l) = FORCE(l) - s * FSSAtheta * dt * PrevSol(m,q) * Normal(m) &
                                 * Basis(q) * LoadVec(j) 
                      END DO
                    END IF
                  END IF
          ...
          ELSE
            DO i=1,dim
              DO q=1,nd
                ...
                ELSE
                  DO m=1,dim
                    FORCE(k) = FORCE(k) - s * FSSAtheta * dt * PrevSol(m,q) * Normal(m) &
                             * Basis(q) * LoadVec(i)
                  END DO
                END IF
              END DO
      ...
      IF ( HaveFSSA ) THEN
        ...
        CASE ('transposed')
          DO p=1,nd
            ...
            END DO
          END DO        
        CASE ('full','adapted') ! full entry matrix FSSA
          DO p=1,nd
            DO q=1,nd
              DO i=1,dim
                DO j=1,dim
